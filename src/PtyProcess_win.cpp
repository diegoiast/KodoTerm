    ...
    void run() override {
        ...
        while (m_running) {
            if (ReadFile(m_hPipe, buffer, sizeof(buffer), &bytesRead, NULL) && bytesRead > 0) {
                ...
                QMetaObject::invokeMethod(m_parent, "onReadThreadData", Qt::QueuedConnection, Q_ARG(QByteArray, data));
            } else {
                break;
            }
        }
...
bool PtyProcessWin::start(const QString &program, const QStringList &arguments, const QSize &size) {
    ...
    // Create pipes
    if (!CreatePipe(&hPipePTYIn, &m_hPipeOut, NULL, 0)) return false;
    if (!CreatePipe(&m_hPipeIn, &hPipePTYOut, NULL, 0)) return false;
    ...
    if (!InitializeProcThreadAttributeList(si.lpAttributeList, 1, 0, &bytesRequired)) return false;
    
    if (!UpdateProcThreadAttribute(si.lpAttributeList, 0, PROC_THREAD_ATTRIBUTE_PSEUDOCONSOLE, m_hPC, sizeof(HPCON), NULL, NULL)) {
        return false;
    }
...
void PtyProcessWin::onReadThreadData(const QByteArray &data) {
    emit readyRead(data);
}
